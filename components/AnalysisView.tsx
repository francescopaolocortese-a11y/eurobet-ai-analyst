...
import { ExternalLink, AlertCircle, Sparkles, Target, Trophy, Activity, Bell, BellRing, Wallet, CheckCircle2, XCircle, Clock, TrendingUp, BarChart3 } from 'lucide-react';
...
const StatBar: React.FC<{ label: string; homeValue: number; awayValue: number; homeColor?: string; awayColor?: string; unit?: string; max?: number }> = ({ 
    ...
    // If max is provided, use it for scale, otherwise calculate total
    const total = max ? max : (homeValue + awayValue || 1);
    
    // For xG, we might want independent bars or comparison. Let's stick to comparison for now but handle small numbers.
  ...
  if (loading) {
    return (
      <div className="h-full flex flex-col items-center justify-center p-12 text-zinc-500 animate-pulse">
        ...
        <p className="text-sm mt-2">Integrazione dati live e xG in corso.</p>
      </div>
    );
  }
        ...
        <div className="lg:col-span-1 flex flex-col gap-6">
            ...
            {data.stats && (
                <div className="bg-zinc-900/50 border border-zinc-800 rounded-2xl p-6">
                    <h3 className="text-sm font-bold text-zinc-400 uppercase tracking-wider mb-4 flex items-center gap-2">
                        <BarChart3 size={16} className="text-emerald-500" /> Statistiche Live
                    </h3>
                    ...
                    <div className="grid grid-cols-2 gap-4 mt-4 pt-4 border-t border-zinc-800/50">
                        <div className="text-center">
                            <div className="text-[10px] text-zinc-500 uppercase">Cartellini Gialli</div>
                            <div className="text-lg font-bold text-yellow-500">{data.stats.home.yellowCards} - {data.stats.away.yellowCards}</div>
                        </div>
                        <div className="text-center">
                             <div className="text-[10px] text-zinc-500 uppercase">Cartellini Rossi</div>
                             <div className="text-lg font-bold text-red-500">{data.stats.home.redCards} - {data.stats.away.redCards}</div>
                        </div>
                    </div>
                </div>
            )}
            ...
            <div className="bg-zinc-900/50 border border-zinc-800 rounded-2xl p-6 flex flex-col">
                <h3 className="text-sm font-bold text-zinc-400 uppercase tracking-wider mb-6 flex items-center gap-2">
                    <TrendingUp size={16} /> Probabilità 1X2
                </h3>
                
                <div className="flex flex-col justify-center h-full space-y-4">
                  <div className="flex justify-between items-end text-sm">
                      <div className="text-left flex flex-col items-start gap-1">
                          {match.homeTeamLogo && <img src={match.homeTeamLogo} alt="Home" className="w-6 h-6 object-contain" />}
                          <span className="block font-bold text-white text-base truncate max-w-[100px]">{match.homeTeam}</span>
                          <span className="text-emerald-500 font-black text-2xl">{data.homeWinProb}%</span>
                      </div>
                      <div className="text-center pb-1">
                          <span className="text-zinc-500 font-medium text-xs mb-1 block">Pareggio</span>
                          <span className="block text-zinc-300 font-bold text-lg">{data.drawProb}%</span>
                      </div>
                      <div className="text-right flex flex-col items-end gap-1">
                          {match.awayTeamLogo && <img src={match.awayTeamLogo} alt="Away" className="w-6 h-6 object-contain" />}
                          <span className="block font-bold text-white text-base truncate max-w-[100px]">{match.awayTeam}</span>
                          <span className="text-red-500 font-black text-2xl">{data.awayWinProb}%</span>
                      </div>
                  </div>

                  <div className="h-4 w-full flex rounded-full overflow-hidden bg-zinc-800 ring-1 ring-zinc-800">
                      <div style={{ width: `${data.homeWinProb}%` }} className="bg-emerald-500 h-full transition-all duration-1000 ease-out" />
                      <div style={{ width: `${data.drawProb}%` }} className="bg-zinc-600 h-full transition-all duration-1000 ease-out" />
                      <div style={{ width: `${data.awayWinProb}%` }} className="bg-red-500 h-full transition-all duration-1000 ease-out" />
                  </div>

                  <div className="flex justify-between text-[10px] text-zinc-500 font-medium uppercase tracking-wider px-1">
                      <div className="flex items-center gap-1"><div className="w-2 h-2 rounded-full bg-emerald-500"></div> 1 - Casa</div>
                      <div className="flex items-center gap-1"><div className="w-2 h-2 rounded-full bg-zinc-600"></div> X - Pari</div>
                      <div className="flex items-center gap-1"><div className="w-2 h-2 rounded-full bg-red-500"></div> 2 - Ospite</div>
                  </div>
                </div>
            </div>
